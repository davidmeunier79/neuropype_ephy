...
class SplitWindowsInputSpec(BaseInterfaceInputSpec):
    ...
    n_windows = traits.List(traits.Tuple, desc='List of start and stop points (tuple of two integers) of temporal windows', mandatory = True)
    
class SplitWindowsOutputSpec(TraitedSpec):
    
    win_ts_files = traits.List(traits.File(exists=True), desc="List of files with splitted timeseries by windows")
